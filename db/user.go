...
type User struct {
	...
	Career       string     `gorm:"type:varchar(100)"`
...
func (u *User) VerifyByEmailAndPassWord() (*[]User, error) {
	users := []User{}
	d := DB.Where("email = ? and password = ? and status = 1", u.Email, u.Password).Find(&users)
...
func (u *User) GetUserById() (*User, error) {
	user := &User{}
	d := DB.Where("id = ?", u.Id).Find(user)
	return user, d.Error
}

func (u *User) UpdateStatus() (*User, error) {
	d := DB.Model(&u).Where("id = ?", u.Id).Update("status", u.Status)
	return u, d.Error
}

func (u *User) UpdateUserById() (int64, error) {
	d := DB.Model(&u).Where("id = ?", u.Id).Update(u)
